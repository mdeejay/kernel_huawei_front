...
#define BATTERY_CAPACITY_LOW		2	/* set battery capacity low threshold to 2% */
//#define BATTERY_CAPACITY_NORMAL		20	/* set battery capacity normal threshold to 20% */
...
/*Dont use pmic for charging, so remove the relative configuration*/
#if 0
...
/* Detect the battery present status and read battery info with gas gauge, ac/usb charging done by 
 * charger ic, so remove charge func and interrput func here.
 */
#if 0
...
static enum power_supply_property twl6030_bci_battery_props[] = {
    ...
    // added technology sysfs interface for battery	
...
int notify_mdm_off_to_pm(void)
{
	if (system_state != SYSTEM_RUNNING)
		return 0;
	if ((bq27510_battery_voltage(&dev27510) < 3400000)&&
...
static int capacity_changed(struct twl6030_bci_device_info *di)
{
	...
	int curr_capacity = bq27510_battery_capacity(&dev27510);//di->capacity;
	...
	if((di->usb_online | di->ac_online) && (twl6030_get_gpadc_conversion(10) < BQ2416x_IC_THRESHOLD_VOLTAGE))
	{
		//di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
		di->power_supply_status = POWER_SUPPLY_HEALTH_DEAD;
	}

       /* When capacity is below 15%, hold the wakelock to avoid suspend and release it when above 20%.
	   *  This scene is used to BQ27510 BAT_LOW pin booting a low level.
	   */
    ...
    if (di->capacity == -1) {
       ...
       if (  abs(di->capacity -curr_capacity) >= 1 ){
        if ((abs(di->capacity -curr_capacity) >= 5)&&(curr_capacity == 0))
        {
            if (++di->capacity_debounce_count >= 4)
            {
                 di->capacity = curr_capacity;
                 ...
                 "curr_capacity = %d \n" "curr_voltage = %d \n",
                 di->capacity, curr_capacity,bq27510_battery_voltage(&dev27510));
                 return 1;
            }
			else{
                return 0;
        ...
        if (  abs(di->capacity -curr_capacity) >= 5 ){
				dev_info(di->dev,"last_capacity = %d \n"
	                  "curr_capacity = %d \n", di->capacity, curr_capacity);
			}
				...
				void *_data)
{
    ...
    current_capacity = bq27510_battery_capacity(&dev27510);
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		val->intval = twl6030_get_gpadc_conversion(8);		
		break;
	default:
		return -EINVAL;
	}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = bq27510_battery_voltage(&dev27510);	
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = bq27510_battery_current(&dev27510);	
		break;
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = bq27510_battery_temperature(&dev27510);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = is_bq27510_battery_exist(&dev27510);	
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		val->intval = bq27510_battery_current(&dev27510);//di->current_avg_uA;
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		val->intval = bq27510_battery_health(&dev27510);//di->bat_health;
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
        if (modem_off) {
            val->intval = 0;
        }else {
            val->intval = bq27510_battery_capacity(&dev27510);
        }
		break;
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		val->intval = bq27510_battery_capacity_level(&dev27510);
		break;
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		val->intval = bq27510_battery_technology(&dev27510);
		break;
	case POWER_SUPPLY_PROP_CAPACITY_RM:
		val->intval = bq27510_battery_rm(&dev27510);
		break;
	case POWER_SUPPLY_PROP_CAPACITY_FCC:
		val->intval = bq27510_battery_fcc(&dev27510);
		break;
	default:
		return -EINVAL;
	}
...
/* Dont need register otg notifier, register it in bq2416x device driver for receiving
 * charger's type and excute difficult charging process*/
#if 0	
