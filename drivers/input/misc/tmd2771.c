...
/* wake lock must add in poll mode,or system will sleep if prox happen when calling */
...
/* not need calibrate when first open */
...
static struct file_operations taos_fops = 
{
	...
	/*w00175802 modify for enable light sersor begin*/  
    .unlocked_ioctl = taos_ioctl,
	/**w00175802 modify for enable light sersor end*/  
};
...
als_process:
    if(atomic_read(&taos->l_flag))
    {
        ...
        input_report_abs(taos->input_dev, ABS_MISC, lux);
        //input_report_abs(taos->input_dev, ABS_LIGHT, lux);
		input_sync(taos->input_dev);
    }
...
static int taos_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    enable_power_for_device(&client->dev,PL_POWER_NAME,DEV_POWER);
	...
	input_set_abs_params(taos->input_dev, ABS_MISC, 0, 10240, 0, 0);
...
device_initcall_sync(tmd2771_init);
