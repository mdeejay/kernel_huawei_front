...
//modify FAE
	...
	luxValue = SCALE*((IAC*GA*DF)/100)/(((272*(256-data->atime))/100) *data->als_gain);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
		...
		if (data->enable_ps_sensor==0) {
		    ...
		    apds990x_set_control(client, 0x22); /* 100mA, IR-diode, 1X PGAIN, 1X AGAIN */
		...
		if (data->enable_als_sensor) {
			...
			//modify FAE
			//apds990x_set_control(client, 0x20); /* 100mA, IR-diode, 1X PGAIN, 1X AGAIN */
            apds990x_set_control(client, 0x22); /* 100mA, IR-diode, 1X PGAIN, 16X AGAIN */
            apds990x_set_pers(client, 0x33); /* 3 persistence */
...
{      
    struct apds990x_data *data = i2c_get_clientdata(client);
    unsigned long flags;
    printk("%s: apds9900 try to suspend\n",__func__);
...
{             
    ...
    printk("%s: apds9900 try to resume\n",__func__);    
