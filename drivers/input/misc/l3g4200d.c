				...
				int poll_interval)
{
    ...
    /*dev_err(&gyro->client->dev, "w00185212------l3g4200d_update_odr--poll_interval_ms= %d-----update odr= 0x%x\n",
			poll_interval,config[1]);
			*/
...
#ifdef DEBUG
/*
	 printk(KERN_INFO "gyro_out: y = %d x = %d z= %d\n",
...
static int l3g4200d_hw_init(struct l3g4200d_data *gyro)
{
    ...
    //test begin w00185212
    buf[0] = WHO_AM_I;	
    err = l3g4200d_i2c_read(gyro, buf, 1);
    printk(KERN_INFO "%s hw init-6666- err= %d WHO_AM_I = %x\n", L3G4200D_GYR_DEV_NAME,err,buf[0]);
			       ...
			       const char *buf, size_t size)
{
    ...
    {
        gyro->resume_state[RES_CTRL_REG1] = 0x6F;
	    gyro->resume_state[RES_CTRL_REG2] = 0x00;
	    gyro->resume_state[RES_CTRL_REG3] = 0x00;
	    gyro->resume_state[RES_CTRL_REG4] = 0xA0;
	    gyro->resume_state[RES_CTRL_REG5] = 0x02;
	...
	msleep(800);
        ...
        {
            //printk("selftest_gyro-------MAX-MIN--X-==%d\n",data);
            return ERR_SELFTEST;
        }
       data = abs(data_out.y);
        if((data > MAX_VAL) || (data < MIN_VAL))
        {
            //printk("selftest_gyro-------MAX-MIN--Y-==%d\n",data);
            return ERR_SELFTEST;    
        }
       data = abs(data_out.z);
        if((data > MAX_VAL) || (data < MIN_VAL))
        {
            //printk("selftest_gyro-------MAX-MIN--Z-==%d\n",data);
            return ERR_SELFTEST;    
        }
...
static void l3g4200d_input_work_func(struct work_struct *work)
{
	...
	int err;
//	pr_err("%s: l3g4200d_input_work_func start.\n", L3G4200D_GYR_DEV_NAME);
...
{
	int err;
	pr_err("l3g4200d_input_init---START---w00185212");
//test
					...
					const struct i2c_device_id *devid)
{
    ...
    enable_power_for_device(&client->dev,GYRO_POWER_NAME,DEV_POWER);
