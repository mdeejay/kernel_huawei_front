...
#if 0
#define VAUX2_POWR_VOLTAGE 2800000 //2.8v
static    struct regulator        *vaux2 = NULL;

 int enable_vaux2_power_for_synaptics(struct device * dev , const char * id)
{
    int voltage , ret;
    ret = 0 ;
    TS_DEBUG_RMI("******dev_name = %s\t,id = %s\t******\n ",dev_name(dev),id);

    if(vaux2 == NULL)
    vaux2 = regulator_get(dev, id);
    if (IS_ERR(vaux2))
    {
        printk(KERN_INFO"turn_on_aux2 for dev(%s) id(%s) error", dev_name(dev), id);
        vaux2 = NULL;
        return -ENODEV;
    }
    voltage =    regulator_get_voltage(vaux2);
    if (voltage != VAUX2_POWR_VOLTAGE)
    {
        printk(KERN_INFO"vaux2 set voltage 2.8v \n");
        regulator_set_voltage(vaux2 ,  2800000 , 2800000 );
    }
    printk(KERN_INFO"turn_on_aux2 for dev(%s) id(%s)",  dev_name(dev), id );
    ret =    regulator_enable(vaux2);
    TS_DEBUG_RMI("******aux2__power = %d******\n ",ret);
    ...
    struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // enable_vaux2_power_for_synaptics( &client->dev, "synaptics_ts_power");
#if 0
    gpio_request(TOUCH_OMAP4430_RESET_PIN,"TOUCH_RESET");
    gpio_direction_output(TOUCH_OMAP4430_RESET_PIN, 1);
    mdelay(5);
...
err_check_functionality_failed:
    ...
    {
        #if 0
        regulator_disable(vaux2);
        regulator_put(vaux2);
        printk(KERN_ERR "Turn off the power!\n ");
        #endif
    }
...
static int i2c_update_firmware(struct i2c_client *client)
{
    ...
    uint16_t    length;
    int ret = 0;
    const char filename[]="/sdcard/update/synaptics.img";//we can use adb push to store the img
    ...
    printk("^^^^^^^^i2c update firmware begin^^^^^^^^\n");
    ...
    printk("^^^^^^^^i2c update firmware end ^^^^^^^^\n");
...
{
    char ret = -1;
    printk("###### update_firmware_store enter ######\n");
    printk("###### buf[0] = %d ######\n", buf[0],buf[1]);
