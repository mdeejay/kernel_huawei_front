...
static int nve_do_index_table(struct NVE_struct *nve)
{
	...
	printk("[NVE] current_id = %d nve_age = %d\n",nve->nve_current_id,nve_header->nve_age);
	return 0;
}
...
static int nve_open(struct inode *inode, struct file *file)
{
	...
	printk("nve_open in\n");
//kernel3.0 have not these function, and CONFIG_LOCK_KERNEL is not enable in android_4430_defconfig, so these two function have not use.    
	//lock_kernel();
    ret = nve_open_ex();
	//unlock_kernel();
