...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	if (cpu_load >= go_hispeed_load) {
		...
		} else if (lpspeed_enabled && pcpu->policy->cur <= hispeed_freq) {
		    /* if lpspeed_enabled, go to lpspeed_freq first */
	        new_freq = min(lpspeed_freq, pcpu->policy->max);
	        pcpu->lpspeed_count = 0;
		} else if (lpspeed_enabled && pcpu->policy->cur <= lpspeed_freq
		        && pcpu->lpspeed_count <= lpspeed_count_threshold - 1) {
		    /* keep cpu freq to lpspeed_freq for lpspeed_count_threshold times */
	        new_freq = min(lpspeed_freq, pcpu->policy->max);;
            pcpu->lpspeed_count++;
		} else {
