/*changge the whole file to compare the synptices touch panel*/
...
//#define TK_LED_GPIO 137
//#define LED_UP   1
/*use TS_RMI_DEBUG to contrl the debug message*/
...
/* add the rezero addr */
#define FUCNTION_REZERO   0xFB
/* init the rezero as 0 */
static int rezero = 0;
...
static int synaptics_debug_mask = 0;//modify initialize number to control whether show the debug message! eg: 0 -- close; 1 -- open  
	...
	for (fd_reg = FD_ADDR_MAX; fd_reg >= FD_ADDR_MIN; fd_reg -= FD_BYTE_COUNT)     
    {
		...
		switch (fd.functionNumber) {
			...
			case 0x11: /* 2D */
                ...
                ts->f11_has_Sensitivity_Adjust = (query[1] >> 6) & 1;//if f11_has_Sensitivity_Adjust=1£¬register map can set bit f11_2d_ctrl14
				egr = &query[7];// egr point to F11_2D_Query7
				...
				TS_DEBUG_RMI("***from PDT*** max X: %d; max Y: %d\n", ts->f11_max_x, ts->f11_max_y);
		...
		if (ts->hasF11 && interrupt[ts->f11.interrupt_offset] & ts->f11.interrupt_mask) 
        {
            ...
            if(ts->is_support_multi_touch)
            {
                ...
                for (f = 0; f < ts->f11.points_supported; ++f) 
                {
                	...
                	wy = finger_reg[3] / 0x10;
                	z = finger_reg[4];
					/* when the finger relese start rezero */
                    if(rezero && (!finger_status_reg))
                    {
                        
                        ret = i2c_smbus_write_byte_data(ts->client, FUCNTION_REZERO, 0x00);
                        if(ret < 0)
                        {
                            printk(KERN_ERR "synaptics_ts_suspend: the touch rezero failed! \n");
                        }
                        else
                        {
                            TS_DEBUG_RMI ("synaptics_ts_suspend: the touch rezero OK! \n");
                        }
...
/* modify for u8600 */
					if(/*machine_is_msm7x27_u8650() || machine_is_msm7x27_c8650() || machine_is_msm7x27_m865()*/0)//jKF31740 Modify
                    {
                    ...
                    if (0 != finger_status)
                    {
                       	dev_dbg(&ts->input_dev->dev, "x = %2d , y = %2d \tthe status is %d\n",x, y,finger_status);
            ...
            else /* else with "if(ts->is_support_multi_touch)"*/
            {
                ...
                dev_dbg(&ts->input_dev->dev,"the finger_status is %2d!\n",finger_status);
...
/* modify for u8600 */
            	...
            	dev_dbg(&ts->input_dev->dev, "the x_sig is %2d ,the y_sig is %2d \n",x, y);
...
out:
	if (ts->use_irq)
	{
	      /* try to clear pending status get rid of risk  */

	/* don't use this code anymore delete 5 lines code */
	...
	char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	/*delete some extra ID of touch panel of synaptics*/
	...
	"Customer Family:%x\n"
	"Firmware Revision:%x\n",
	"synaptics",query_name[0], query_name[1], query_name[2], query_name[3]);
...
static int synaptics_ts_power(struct i2c_client *client, int on)
{
    ...
    {       
		...
		if (ret < 0)
        {
        	printk(KERN_INFO "synaptics sensor can not wake up\n");
        }      
        ...
        if (ret < 0)
        {
            printk(KERN_INFO "synaptics chip can not reset\n");
        }
	...
	else 
    {
		...
		if (ret < 0)
        {
            printk(KERN_INFO "synaptics touch can not enter very-low power state\n");
        }      
	}
	return ret;	
}
    ...
    enum led_brightness value)
{
    if (!value)
    {
        gpio_set_value(TK_LED_GPIO, 0);
    }
    else
    {
        gpio_set_value(TK_LED_GPIO, 1);
    }
    return;
}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct synaptics_i2c_platform_data *touch_pdata = NULL;
...
/* delete one line,needn't control avdd in driver */
    ...
    if(NULL == touch_pdata)
    {
        printk(KERN_ERR "the touch_pdata is NULL please check the init code !\n");
        ret = -ENOMEM;
    ...
    if(touch_pdata->power_on)
    {
        ret = touch_pdata->power_on(&client->dev);
    }    
    if(ret)
    {
    ...
    ret = i2c_smbus_write_byte_data(client, fd_01.controlBase, 0x00); //use control base to set tp wakeup
    if(ret < 0)
    {
        printk(KERN_ERR "synaptics_ts_probe: the touch can't resume! \n");
    }
    mdelay(50);   
   ...
   ret = i2c_smbus_write_byte_data(client, fd_11.controlBase + 2, 0x06); 
    if(ret < 0)
    {
        printk(KERN_ERR "synaptics_ts_probe: set the threshold faild! \n");
    }
    ret = i2c_smbus_write_byte_data(client, fd_11.controlBase + 3, 0x06); 
    if(ret < 0)
    {
        printk(KERN_ERR "synaptics_ts_probe: set the threshold faild! \n");
    }
	...
	d_entry = create_proc_entry("tp_hw_type",
	...
	if (ts->hasF11) {
		for (i = 0; i < ts->f11.points_supported; ++i) {
          if(ts->is_support_multi_touch)
          {
            ...
            /* reduce the range of report value */
	...
	if (client->irq) {
		...
		if (request_irq(client->irq, synaptics_rmi4_irq_handler,
			...
			TS_DEBUG_RMI("***synaptics_request_irq***: request_irq input_gpio =  %d \n",irq_to_gpio(client->irq));
            TS_DEBUG_RMI("***synaptics_request_irq***: irq number %d\n",client->irq);
...
#if 0
       ret = gpio_request(TK_LED_GPIO, "tk_bk_led");
    if (ret < 0) {
        printk(KERN_ERR "unable to get INT GPIO\n");
        ret = -ENODEV;
        goto err_key_input_register_device_failed;
    }
       gpio_direction_output(TK_LED_GPIO, LED_UP);

       ret = led_classdev_register(&client->dev, &Tk_bl_led);
    if (ret) {
        printk(KERN_ERR "unable to register led class driver\n");
        gpio_free(TK_LED_GPIO);
    }
#endif
	return 0;
...
err_get_version_failed:
    if (touch_pdata->power_off)
    {
        ret = touch_pdata->power_off();
        {
            printk(KERN_ERR "the power is off: %d \n ", ret);   
        }
    }
err_power_on_failed:
...
static int synaptics_rmi4_remove(struct i2c_client *client)
{
	...
	kfree(ts);
//    led_classdev_unregister(&Tk_bl_led);
    if(touch_pdata->power_off)
    {
        touch_pdata->power_off();
    }
	return 0;
}
...
/* if use interrupt disable the irq ,else disable timer */ 
	...
	/* TURN OFF THE REZERO FUCNTION */
    ret = i2c_smbus_write_byte_data(client, FUCNTION_REZERO, 0x02);
    if(ret < 0)
    {
        printk(KERN_ERR "synaptics_ts_suspend: the touch turn off rezero failed! \n");
    }
	...
	ts->enable = 0;
	/* set the rezero flag as first touch */
    rezero = 1;
	return 0;
}
...
static int RMI4_read_PDT(struct i2c_client *client)
{
	...
	for(start_addr = 0xe9; start_addr > 10; start_addr -= sizeof(struct RMI4_FDT))
	{
		...
		{
			SYNAPITICS_DEBUG("%s:%d: read RIM4 PDT error!\n",__FUNCTION__,__LINE__);
			return -1;
		}
	...
	{
		SYNAPITICS_DEBUG("%s:%d: RIM4 PDT has changed!!!\n",__FUNCTION__,__LINE__);
		
		/* when the tp come into flash mode the contrler maybe changged so we need read them again */
		ret = synaptics_rmi4_read_pdt(ts);
		if(ret < 0)
		{
			SYNAPITICS_DEBUG("read pdt error:!\n");
			return -1;
		}
...
static int RMI4_program_firmware(struct i2c_client *client,const unsigned char *pgm_data)
{
	...
	SYNAPITICS_DEBUG("RMI4 program UI firmware...\n");
...
static int synaptics_download(struct i2c_client *client,const unsigned char *pgm_data)
{
	...
	if(ret != 0)
	{
		SYNAPITICS_DEBUG("RMI page func check error\n");
		return -1;
	}
	...
	if( ret != 0)
	{
		SYNAPITICS_DEBUG("%s:%d:RMI enable program error,return...\n",__FUNCTION__,__LINE__);
		goto error;
	}
	...
	if( ret != 0)
	{
		SYNAPITICS_DEBUG("%s:%d:RMI check firmware error,return...\n",__FUNCTION__,__LINE__);
		goto error;
	}
	...
	if( ret != 0)
	{
		SYNAPITICS_DEBUG("%s:%d:RMI program firmware error,return...",__FUNCTION__,__LINE__);
		goto error;
	}
...
static int i2c_update_firmware(struct i2c_client *client) 
{
    ...
    const char filename[]="/data/synaptics.img";
    const char filename_bak[]="/data/synaptics.img";
	SYNAPITICS_DEBUG("i2c update firmware begin\n");
	...
	if (!filp->f_op)
	{
            SYNAPITICS_DEBUG("%s: File Operation Method Error\n", __FUNCTION__);
    ...
    if (!inode) 
    {
        SYNAPITICS_DEBUG("%s: Get inode from filp failed\n", __FUNCTION__);
        filp_close(filp, NULL);
        set_fs(oldfs);
        return -1;
    }
    ...
    {
        SYNAPITICS_DEBUG("file size error\n");
        filp_close(filp, NULL);
        set_fs(oldfs);
        return -1;
    }
	...
	if (!buf) 
	{
		SYNAPITICS_DEBUG("alloctation memory failed\n");
		filp_close(filp, NULL);
		set_fs(oldfs);
		return -1;
	}
    ...
    {
        SYNAPITICS_DEBUG("%s: file read error\n", __FUNCTION__);
        filp_close(filp, NULL);
        set_fs(oldfs);
        vfree(buf);
        return -1;
	...
	SYNAPITICS_DEBUG("i2c update firmware end\n");
...
static int ts_firmware_file(void)
{
	...
	if (!kobject_ts)
	{
		SYNAPITICS_DEBUG("create kobjetct error!\n");
		return -1;
	}
	...
	if (ret) {
		...
		SYNAPITICS_DEBUG("create file error\n");
		return -1;
	}
	return 0;	
}
...
{
	char ret = -1;
	SYNAPITICS_DEBUG("###### update_firmware_store enter ######\n");
    SYNAPITICS_DEBUG("###### buf[0] = %d ######\n", buf[0],buf[1]);

	if (buf[0] == '2')//ehco "2">>object to start firmware update
	{
		/* driver detect its device  */
		ret = i2c_smbus_read_byte_data(g_client, fd_01.queryBase);
		SYNAPITICS_DEBUG("The if of synaptics device is : %d\n",ret);
		...
		if( 0 != ret )
		{
			SYNAPITICS_DEBUG("Update firmware failed!\n");
			ret = -1;
		} 
		else 
		{
			SYNAPITICS_DEBUG("Update firmware success!\n");
