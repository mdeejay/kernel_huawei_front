...
static void adxl34x_service_ev_fifo(struct adxl34x *ac)
{
    ...
    //printk("x=%d, y=%d, z=%d", adxl34_data[0],adxl34_data[1],adxl34_data[2]);
}
...
static int __devinit adxl34x_initialize(bus_device *bus, struct adxl34x *ac)
{
    ...
    {
        err = pdata->gpio_config_interrupt();
        if(err)
        {
...
static void adxl34x_early_suspend(struct early_suspend *h)
{
    ...
    dev_info(&ac->bus->dev,"adxl34x_model has suspend\n"); 
}
...
static void adxl34x_late_resume(struct early_suspend *h)
{
    ...
    dev_info(&ac->bus->dev,"adxl34x_model has resumed\n"); 
}
#endif
