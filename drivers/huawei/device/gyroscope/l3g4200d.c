...
static int l3g4200d_hw_init(struct l3g4200d_data *gyro)
{
    ...
    buf[0] = WHO_AM_I;
    err = l3g4200d_i2c_read(gyro, buf, 1);
    dev_dbg(&gyro->client->dev, "%s hw init-6666- err= %d WHO_AM_I = %x\n", L3G4200D_GYR_DEV_NAME,err,buf[0]);
...
{
    struct l3g4200d_data *gyro;
    struct l3g4200d_gyr_platform_data *devpd = client->dev.platform_data;
    int err = -1;
    ...
    if(devpd->get_regulator)
    {
        ...
        if(err < 0){
            printk(KERN_ERR "Gyro_probe: get power fail!\n");
        goto err0;
        }
    }
    msleep(5);
