...
static void apds990x_change_als_threshold(struct i2c_client *client)
{
	...
	if ( ((data->ps_detection == 1) && (irdata > 5500) && (board_id != 0)) ||
	                       ((data->ps_detection == 1) && (cdata > (75*(1024*(256-data->atime)))/100) && (board_id == 0)))	// PS was previously in far-to-near condition
	{
...
static void apds990x_work_handler(struct work_struct *work)
{
	...
	if ((status & data->enable & 0x30) == 0x30) {
		...
		cdata = i2c_smbus_read_word_data(client, CMD_WORD|APDS990x_CDATAL_REG);
		if ((irdata < 5500 && (board_id != 0)) || ((cdata < (75*(1024*(256-data->atime)))/100) && (board_id == 0))) 
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
		...
		if (data->enable_als_sensor) {
			...
			apds990x_set_enable(client,0); /* Power Off */
			if( board_id != 0 ) apds990x_set_atime(client, 0xdb);
			else apds990x_set_atime(client, 0x6c);
			apds990x_set_ailt( client, 0);
			apds990x_set_aiht( client, 0xffff);
				   ...
				   const struct i2c_device_id *id)
{
	...
	int err = 0;
       flag_work_enable = 1;
    board_id = get_proximity_light_boardId_value();
    if( board_id == 1 )
    {
        apds_990x_pwindows_value = 150;
	...
	if (get_mhl_ci2ca_value() == 0)
	{
		// If the MHL is pulled down, the MHL and this light sensor's 
		// I2C addresses conflicts with each other, we choose to disable
		// light sensor in this case.
		return -EIO;
	}
	...
	{
	    platform_data->gpio_config_interrupt();
	}
