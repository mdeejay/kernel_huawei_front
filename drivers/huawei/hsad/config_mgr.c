...
/*get int board id*/
int get_board_id_int(void)
{
	
	return (unsigned short)((g_board_id.id0 <<8) | (g_board_id.id1 <<4) | g_board_id.id2);
}

/*setup.c call set_hw_config,init list*/
int set_hw_config(board_id_t a_board_id)
{
	...
	g_current_board_id = get_board_id_int();
	...
	else
	{
		...
		char arr[32];
		bool flag;
		get_hw_config_string("lcd/name",arr,32,NULL);
		HW_CONFIG_DEBUG("get_hw_config_string lcd/name : %s\n",arr);
		get_hw_config("lcd/name",arr,32,NULL);
		HW_CONFIG_DEBUG("get_hw_config(string) lcd/name : %s\n",arr);
		get_hw_config_int("lcd/init_index",&a,NULL);
		HW_CONFIG_DEBUG("get_hw_config_int lcd/init_index : %d\n",a);
		get_hw_config("lcd/init_index", &a, sizeof(unsigned int), NULL); 
		HW_CONFIG_DEBUG("get_hw_config(int) lcd/init_index : %d\n",a);

		get_hw_config_bool("lcd/sub_sample", &flag,  NULL); 
		HW_CONFIG_DEBUG("get_hw_config(int) lcd/sub_sample : %d\n",flag);
		
		HW_CONFIG_DEBUG("get_board_id_int : 0x%x\n",get_board_id_int());
...
/*gpio get gpio struct by name*/
gpiomux_setting *get_gpio_struct_by_name(char * name)
{
	...
	while(MUX_PIN_END != gpio_ptr->mux_pin )
	{
			...
			if(!strcmp(gpio_ptr->net_name, name))
			{
				
				return gpio_ptr;
			}
			if(!strcmp(gpio_ptr->general_name, name))
			{
...
{
	gpiomux_setting *gpio_ptr = get_gpio_struct_by_name(name);
	if(NULL != gpio_ptr)
	{
		return gpio_ptr->gpio_num;
	}
	else
	{
