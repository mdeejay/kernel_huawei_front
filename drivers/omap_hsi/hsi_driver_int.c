...
bool hsi_is_hsi_port_busy(struct hsi_port *pport)
{
	...
	/* HSR state machine */
	if (!hsi_driver_is_interrupt_pending(pport, HSI_CAWAKEDETECTED, false)) {
...
dpr_0("hsi_driver_int.c. %s. [flag]0x%08X->[reg]0x%08X;\n",__func__,flag,hsi_inl(pport->hsi_controller->base,
                HSI_SYS_MPU_ENABLE_REG(pport->port_number, pport->n_irq)) );   //#y00185015#20110917#
...
static void hsi_do_channel_rx(struct hsi_channel *ch)
{
	...
	if (hsi_driver_device_is_hsi(to_platform_device(hsi_ctrl->dev))) {
		...
		if( (bool)(ch->hsi_port->hsi_mode&XMM_HSI_USE_BOOT)==false )    //XMM_HSI_USE_BOOT, HSI使用模式标志;
#endif
			dev_dbg(hsi_ctrl->dev,
...
static u32 hsi_process_int_event(struct hsi_port *pport)
{
...

	{
		...
		if (pport->cawake_status != cawake_status) {
			//Add a security to not process CAWAKE here if interrupt is
			//pending because it will be processed soon anyway.
			caw_int = hsi_driver_is_interrupt_pending(pport,
							HSI_CAWAKEDETECTED, false);
			caw_int_u = hsi_driver_is_interrupt_pending(pport,
							HSI_CAWAKEDETECTED, true);
