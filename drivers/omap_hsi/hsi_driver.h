 ...
 * @wake_3_wires_mode: 3 wires mode (1) or 4 wires mode (0)
...
//#if defined(CONFIG_HSI_FLASHLESS_SUPPORT)
	int wake_3_wires_mode;  //bit0-For HSR,CP->AP; 0,4-wire HSR; 1,3-wire HSR; bit1-For HST,AP->CP; 0,4-wire HST; 1,3-wire HST;

	//int cawake_mode;    //cawake_mode, cawake??: 0,"DISABLE"; 1,"ENABLE"
	int hsi_mode;    //hsi_mode, bit0~31;
...
int hsi_driver_disable_interrupt(struct hsi_port *pport, u32 flag);  /* Disables the CAWAKE, BREAK, or ERROR interrupt for the given port */
