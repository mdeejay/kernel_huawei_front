...
int hsi_open(struct hsi_device *dev)
{
	...
	if (port->wake_rx_3_wires_mode)
            hsi_driver_enable_interrupt(port, HSI_ERROROCCURED | HSI_BREAKDETECTED);
        else
...
if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_3_WIRE_TYPE)==true ) //XMM_HSI_3_WIRE_TYPE, HSI线模式标志;
  dpr_2("HARII %s, %d u32, 0x%08x;\n",__func__,size,(u32) (*addr) ); //#20110817#y00185015#
//dpr_3("hsi_driver_if.c::hsi_write::%s ch %d, @%x, size %d u32\n", __func__,dev->n_ch, (u32) addr, size);    //#20110817#y00185015#
...
if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_3_WIRE_TYPE)==true ) //XMM_HSI_3_WIRE_TYPE, HSI线模式标志;
    dpr_2("HARII %s, [Previous] 0x%08x;\n",__func__,(u32) (*addr)); //#20110817#y00185015#
...
//dpr_3("hsi_driver_if.c::hsi_read::%s ch %d, @%x, size %d u32\n", __func__,dev->n_ch, (u32) addr, size);    //#20110817#y00185015#
	if (size == 1)
...
//if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_3_WIRE_TYPE)==true ) {//XMM_HSI_3_WIRE_TYPE, HSI线模式标志;
if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_RD_MDELAY)==true ) //XMM_HSI_RD_MDELAY, HSI hsi_read后mdelay标志;	//##调试而设#
    mdelay(1); //mdelay(1);   //#20110817#y00185015#For Hari#Add little delay:
...
if_dev_dpr_0( (bool)(pport->hsi_mode&XMM_HSI_3_WIRE_TYPE)==true, //XMM_HSI_3_WIRE_TYPE, HSI线模式标志;
            dev->device.parent, "IOCTL: ch %d, command %d, (dev->ch->flags & HSI_CH_OPEN)=true?%d;\n",     // /* #20110913#y00185015#Add for hsr_fifo_flush.patch# */
              channel, command,(dev->ch->flags & HSI_CH_OPEN) );
	...
	case HSI_IOCTL_ACWAKE_UP:
		...
		if ( pport->wake_3_wires_mode & HST_WAKE_WIRES_MASK ) { //Modify based on hsi_3wire.patch.20110915
			dev_warn(dev->device.parent,
				 "ACWAKE up requested in 3 wires mode !\n");
			err = -EPERM;
			goto out;
		}
	...
	case HSI_IOCTL_ACWAKE_DOWN:
		...
		if (pport->wake_3_wires_mode & HST_WAKE_WIRES_MASK ) { //Modify based on hsi_3wire.patch.20110915
			dev_warn(dev->device.parent,
				 "ACWAKE down requested in 3 wires mode !\n");
			err = -EPERM;
			goto out;
		}
	...
	case HSI_IOCTL_GET_ACWAKE:
		...
		*(u32 *)arg = hsi_inl(base, HSI_SYS_WAKE_REG(port));
		break;
	case HSI_IOCTL_FLUSH_RXSTATE:
		hsi_outl(0, base, HSI_HSR_RXSTATE_REG(port));
		break;
	case HSI_IOCTL_FLUSH_TXSTATE:
	...
	case HSI_IOCTL_GET_SPEED:
		...
		*(unsigned long *)arg = hsi_ctrl->hsi_fclk_current;
		break;
	case HSI_IOCTL_SET_3WIRES_MODE:
		dev_info(dev->device.parent, "Entering 3 wires mode\n");
		pport->wake_3_wires_mode = 1;
	pport->wake_3_wires_mode = HSR_WAKE_WIRES_MASK|HST_WAKE_WIRES_MASK; //For HSR,CP->AP; 0,4-wire HSR; 1,3-wire HSR; //For HST,AP->CP; 0,4-wire HST; 1,3-wire HST;   //#y00185015#20110916#Modify based on hsi_3wire.patch.20110915

if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_DISINT_CAWAKE)==true ) //XMM_HSI_DISINT_CAWAKE, HSI CAWAKE中断禁止标志;
    hsi_driver_disable_interrupt(dev->ch->hsi_port, HSI_CAWAKEDETECTED );   //#y00185015#20110917#Add begin for Flashless 3HSI cawake race
		...
		*  IDLE after 1st received frame */
		if (hsi_driver_device_is_hsi(to_platform_device(hsi_ctrl->dev)))
			hsi_set_pm_force_hsi_on(hsi_ctrl);
#if 0   //#if 0   //#else   //#endif
		hsi_outl_or(HSI_SET_WAKE_3_WIRES, base,
			    HSI_SYS_SET_WAKE_REG(port));
#else   //#if 0   //#else   //#endif
        hsi_outl(HSI_SET_WAKE_MODE_3_WIRES(channel), base,
             HSI_SYS_SET_WAKE_REG(port));
#endif   //#if 0   //#else   //#endif

#if 1   //#if 1   //#endif
if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_CLK_UNDIS)==true ) //XMM_HSI_CLK_UNDIS, HSI时钟取消关闭标志;
        /* hnagall -- For tests -- Keepi holding clocks */      //#y00185015#20110921#Add for Hari to debug FlsHSI PSI.3HSI.data_err#
        hsi_clocks_enable_channel(dev->device.parent, channel, __func__);   //#y00185015#20110921#Add for Hari to debug FlsHSI PSI.3HSI.data_err#
#endif  //#if 1   //#endif
		break;
	case HSI_IOCTL_SET_4WIRES_MODE:
		dev_info(dev->device.parent, "Entering 4 wires mode\n");
		pport->wake_3_wires_mode = 0;
if( (bool)(dev->ch->hsi_port->hsi_mode&XMM_HSI_DISINT_CAWAKE)==false ) //XMM_HSI_DISINT_CAWAKE, HSI CAWAKE中断禁止标志;
    hsi_driver_enable_interrupt(dev->ch->hsi_port, HSI_CAWAKEDETECTED );   //#y00185015#20110917#Add begin for Flashless 3HSI cawake race
...
//   HSI_IOCTL_SET_HSI_MODE,    /* 15, Set HSI hsi_mode value for all channel */
        case HSI_IOCTL_SET_HSI_MODE:
            dpr_0(dev->device.parent, "pport->hsi_mode 0x%08x -> 0x%08x \n", pport->hsi_mode,*(int*)arg);
            pport->hsi_mode = *(int*)arg;
            goto out;
            break;
//   HSI_IOCTL_GET_HSI_MODE,    /* 16, Get HSI hsi_mode value for all channel */
        case HSI_IOCTL_GET_HSI_MODE:
            //dpr_1(dev->device.parent, "pport->hsi_mode = 0x%08x \n", pport->hsi_mode);
            *(int*)arg = pport->hsi_mode;
            goto out;
            break;
