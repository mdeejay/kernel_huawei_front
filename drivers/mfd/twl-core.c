...
#define USE_Gernelal_TWL6030_ON_OMAP4430_ENABLE 1
#else   
#define USE_Gernelal_TWL6030_ON_OMAP4430_ENABLE 0
#endif
...
add_children(struct twl4030_platform_data *pdata, unsigned long features)
{
	...
	if (twl_has_regulator() && twl_class_is_6030() &&
		...
		/* Add SMPS to platfrom*/
		child = add_regulator(TWL6030_REG_V1V29, pdata->v1v29,
		                        features);
		if (IS_ERR(child))
			return PTR_ERR(child);
		...
		child = add_regulator(TWL6030_REG_VDD1, pdata->vcore1,
		                        features);
		if (IS_ERR(child))
			return PTR_ERR(child);

		child = add_regulator(TWL6030_REG_VDD2, pdata->vcore2,
		                        features);
		if (IS_ERR(child))
			return PTR_ERR(child);
			
		child = add_regulator(TWL6030_REG_VDD3, pdata->vcore3,
		                        features);
		if (IS_ERR(child))
			return PTR_ERR(child);
	}
...
static void _init_twl6030_settings(void)
{
    ...
    /* This is a workaround only for VIVA T2 and FRONT T1.5, including previous hardware versions */
        /*viva before vn1.5,we should write 0x05,after vn1.5(incluld vn1.5),we should write 0x01.
        front before vn1,we should write 0x05,after vn1(incluld vn1),we should write 0x01.
        */
        if (get_sysen_value())
        {
        ...
        {
        	twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x05, 0xB4);
        }
	if ((!cpu_is_omap446x()) && (!USE_Gernelal_TWL6030_ON_OMAP4430_ENABLE))
	...
	if (cpu_is_omap446x()) {
  		...
  		//removed to clear vmem and vcore3 group register.
	...
	//twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x06, 0x25);
        //twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x30, 0x25);
}
...
static int twl_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x00, 0x64);//add by w00185212 for mhl
...
static int twl_resume(struct i2c_client *client)
{
	...
	twl_i2c_write_u8(TWL6030_MODULE_ID0, 0x01, 0x64);//add by w00185212 for mhl
	u8 rd_reg = 0;
	/* MISC1(0xE4)*/
	twl_i2c_read_u8(TWL6030_MODULE_ID0, &rd_reg, 0xE4);
	rd_reg = rd_reg | 0x07;/*enable VAC_MEAS(0x04), VBAT_MEAS(0x02), BB_MEAS(0x01)*/
	twl_i2c_write_u8(TWL6030_MODULE_ID0, rd_reg, 0xE4);
	/* USB_VBUS_CTRL_SET(0x04)*/
	twl_i2c_read_u8(TWL_MODULE_USB, &rd_reg, 0x04);
	rd_reg = rd_reg | 0x01;/*enable VBUS_MEAS(0x01)*/
	twl_i2c_write_u8(TWL_MODULE_USB, rd_reg, 0x04);
	/* USB_ID_CRTL_SET(0x06)*/
	twl_i2c_read_u8(TWL_MODULE_USB, &rd_reg, 0x06);
	rd_reg = rd_reg | 0x01;/*enable ID_MEAS(0x01)*/
	twl_i2c_write_u8(TWL_MODULE_USB, rd_reg, 0x06);
	return 0;
}
#else
...
twl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*wangguanglin 20110906 add for twl6030 settings for bring up small operation system*/
	if (twl_class_is_6030()) {
