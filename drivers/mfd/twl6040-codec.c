...
static int twl6040_debug=0;
module_param(twl6040_debug,int,S_IRUGO|S_IWUSR);
#define TWL6040_PRINTK(x...)  do { if(twl6040_debug) { printk(x);} } while(0)
				       ...
				       int naudint)
{
	...
	do {
		...
		//mdelay(10);
		TWL6040_PRINTK("start wait_for_completion_timeout\n");
	  init_completion(&twl6040->ready);		
		gpio_set_value(twl6040->audpwron, 1);

		time_left = wait_for_completion_timeout(&twl6040->ready,
							msecs_to_jiffies(700));
		TWL6040_PRINTK("end wait_for_completion_timeout time_left:%d\n",time_left);							
		if (!time_left) {
			intid = twl6040_reg_read(twl6040, TWL6040_REG_INTID);
			if (!(intid & TWL6040_READYINT)) {
				dev_err(twl6040->dev,
