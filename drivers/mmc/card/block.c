	...
	struct mmc_blk_request *brq)
{
	...
	for (retry = 2; retry >= 0; retry--) {
		if (card->host->ops->get_cd && card->host->ops->get_cd(card->host) ==0)
		{
			err = -1;
			break;
	  	}
...
static int mmc_multi_block_read_error_count = 0;
static int mmc_multi_block_write_error_count = 0;
		...
		if (brq.data.error) {
			...
			if (rq_data_dir(req) == READ) {
				if (brq.data.blocks > 1) {
					...
					mmc_multi_block_read_error_count++;

					pr_warning("%s: retrying using single block read,mb_read_error_count=%d\n",
						req->rq_disk->disk_name,mmc_multi_block_read_error_count);
					disable_multi = 1;
					continue;
				}
				...
				/*MMC card is type 0, SD is type 1*/
				if ((brq.data.blocks > 1) && (card->type == 0)) {
					/* Redo read one sector at a time */
					mmc_multi_block_write_error_count ++;

					pr_warning("%s: retrying using single block read,mb_write_error_count=%d\n",
						req->rq_disk->disk_name,mmc_multi_block_write_error_count);
					disable_multi = 1;
					continue;
				}
