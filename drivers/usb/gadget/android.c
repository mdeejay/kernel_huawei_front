 ...
 * History :
   1. 2011-8-12 d00191109
      Fix the android's bug of handling disabling rndis
      Use different PID based on nv value
 */
...
#ifdef CONFIG_HUAWEI_USB_GADGET_SWITCH_SOLUTION
#include <mach/hw_usb_products.h>
...
static char vendor_name[] = "Huawei Incorporated";
static char product_name[] = "Android Adapter";
...
static u8 nv_value = NV_NORMAL;   				// overridden by nv_read()
static u8 is_tmobile = 0;                       // overridden by nv_read()
...
static int hw_modem__function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	/* nothing to do - initialization is handled by mtp_function_init */
	return 0;
}

static void hw_modem__function_cleanup(struct android_usb_function *f)
{
	/* nothing to do - cleanup is handled by mtp_function_cleanup */
}

static int hw_modem__function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return fserial_modem_bind_config(c);
}

static struct android_usb_function hw_modem_function = {
    .name          = "modem",
    .init          = hw_modem__function_init,
    .cleanup       = hw_modem__function_cleanup,
    .bind_config   = hw_modem__function_bind_config,
};

static int hw_nmea_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	/* nothing to do - initialization is handled by mtp_function_init */
	return 0;
}

static void hw_nmea_function_cleanup(struct android_usb_function *f)
{
	/* nothing to do - cleanup is handled by mtp_function_cleanup */
}

static int hw_nmea_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return fserial_nmea_bind_config(c);
}

static struct android_usb_function hw_nmea_function = {
	.name		= "nmea",
	.init		= hw_nmea_function_init,
	.cleanup	= hw_nmea_function_cleanup,
	.bind_config	= hw_nmea_function_bind_config,
};

static int hw_diag_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	/* nothing to do - initialization is handled by mtp_function_init */
	return 0;
}

static void hw_diag_function_cleanup(struct android_usb_function *f)
{
	/* nothing to do - cleanup is handled by mtp_function_cleanup */
}

static int hw_diag_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return fserial_diag_bind_config(c);
}

static struct android_usb_function hw_diag_function = {
	.name		= "diag",
	.init		= hw_diag_function_init,
	.cleanup	= hw_diag_function_cleanup,
	.bind_config	= hw_diag_function_bind_config,
};
#endif
#endif
...
static int product_matches_functions(struct android_usb_product *p, struct android_dev *dev)
{
	struct android_usb_function		*f;
    int i = 0;
    ...
    list_for_each_entry(f, &dev->enabled_functions, enabled_list) {    
		...
		if (product_has_function(p, f) == 0)
			return 0;
	}

    if(p->num_functions == i){
	    return 1;
    }else{
        return 0;
    }
...
static int get_product_id(struct android_dev *dev)
{
	...
	int i;
    int product_id = INVALID_PID;


	if (p) {
		for (i = 0; i < count; i++, p++) {
			if (product_matches_functions(p, dev))
            {              
                product_id = p->product_id;
                // Several different PID shares the same functions
                // need to choose one PID according nv value
                if ((product_id == TMO_ADB_DIAG_PCUI_MS_PID)
                    || (product_id == TMO_MICROSOFT_ADB_DIAG_PCUI_MS_PID)
                    || (product_id == HW_ADB_DIAG_PCUI_MS_PID)
                    || (product_id == HW_MICROSOFT_ADB_DIAG_PCUI_MS_PID)
                    )
                {
                    if (nv_value == NV_MICROSOFT_AUTHENICATION)
                    {
                        if (is_tmobile)
                        {
                            product_id = TMO_MICROSOFT_ADB_DIAG_PCUI_MS_PID;
...
/** *@ Description: Get the pointer of the product using the product's PID
* @param INPUT product_id - the product id of the desired product
* @return pointer of the product whose PID equals the input, or NULL if no match */

struct android_usb_product * get_product_from_pid(int product_id)
{
...
#if 0
static int product_id_before_rndis_enable = HW_PCUI_MS_PID;

static void save_pid_before_rndis_enable(int product_id)
{
    product_id_before_rndis_enable = product_id;
}

static int retrieve_pid_before_rndis_enable(void)
{
    return product_id_before_rndis_enable;
}
#endif
