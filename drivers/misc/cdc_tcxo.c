...
static int cdc_tcxo_set_clock_buf_force_out(int clk_id)
{
	cdc_tcxo_set_req_int(clk_id, 1);
	cdc_tcxo_set_req_prio(clk_id, CDC_TCXO_PRIO_REQINT);
	return 0;
}
				...
				const struct i2c_device_id *id)
{
	...
	if (board_id == BOARD_ID_VIVA || board_id == BOARD_ID_FRONT)
		{
		cdc_tcxo_set_clock_buf_force_out(CDC_TCXO_CLK4);
		}
	return 0;
}
