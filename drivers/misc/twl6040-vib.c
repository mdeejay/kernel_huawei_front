...
#define VIBRATOR_CONFIG_NAME_LENGTH 10
enum {
   VIB_DRIVER_6040 =0x01,
   VIB_DRIVER_AUX3 =0x02,
   VIB_DRIVER_BOTH =VIB_DRIVER_6040 | VIB_DRIVER_AUX3,
};

#define VIBRITOR_DRIVER_6040_STR    "6040"
#define VIBRITOR_DRIVER_AUX3_STR   "AUX3"
#define VIBRITOR_DRIVER_BOTH_STR   "BOTH"

static int g_vib_driver=VIB_DRIVER_BOTH;

int get_vibrator_driver_config(void)
{
    char config_name[VIBRATOR_CONFIG_NAME_LENGTH] = "unknown";

    if(get_hw_config("audio/vibrator_driver",config_name,VIBRATOR_CONFIG_NAME_LENGTH, NULL))
    {
    ...
    if(strcmp(VIBRITOR_DRIVER_6040_STR,config_name)==0)
    {
        g_vib_driver=VIB_DRIVER_6040;
    }
    else  if(strcmp(VIBRITOR_DRIVER_AUX3_STR,config_name)==0)
    {
        g_vib_driver=VIB_DRIVER_AUX3;
    }
    else
    {
        g_vib_driver=VIB_DRIVER_BOTH;
    }
...
void aux3_output_on(void)
{
   if(misc_data->vaux3)
        consumer_regulator_enable(misc_data->vaux3);
   else
     printk("%s Error!\n",__func__);
}

void aux3_output_off(void)
{
    if(misc_data->vaux3){
         consumer_regulator_disable(misc_data->vaux3);
      }
    else
      printk("%s Error!\n",__func__);
}
...
static void vib_set(int const new_power_state)
{
	...
	if (new_power_state)
       {
                      if( g_vib_driver & VIB_DRIVER_AUX3)
                      {
                              aux3_output_on();
                      }

                      if(g_vib_driver & VIB_DRIVER_6040)
                      {
                            ...
                            /*add TWL6040_VIBCTRLLP for TWL6040_REG_VIBCTLL and TWL6040_REG_VIBCTLR register state*/
                            /*
...
static int vib_probe(struct platform_device *pdev)
{
  ...
  data->vaux3=NULL;
  data->vaux3 = get_consumer_regulator(&pdev->dev, "MOTOR_MOTOR");

  if (IS_ERR(data->vaux3))
  {
      printk("get regulator failed\n");
      data->vaux3 = NULL;
      ret=-1;
      goto err2;
  }
    ...
    get_vibrator_driver_config();

  /*Reason: We don't want vibra shaking when kernel boot, and codec driver will powerup twl6040 */
