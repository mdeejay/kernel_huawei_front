			...
			struct hdmi_config *cfg)
{
	...
	hdmi_wp_core_interrupt_set(ip_data, HDMI_WP_IRQENABLE_CORE|HDMI_WP_AUDIO_FIFO_UNDERFLOW);
					...
					struct hdmi_core_audio_config *cfg)
{
	...
	r = hdmi_read_reg(hdmi_av_base(ip_data), HDMI_CORE_AV_I2S_CHST0);
	r = FLD_MOD(r, cfg->i2s_cfg.csbit1, 1, 1);
	hdmi_write_reg(hdmi_av_base(ip_data), HDMI_CORE_AV_I2S_CHST0, r);
...
void hdmi_ti_4xxx_transfer_enable(struct hdmi_ip_data *ip_data, bool enable)
{
	REG_FLD_MOD(hdmi_wp_base(ip_data),
...
EXPORT_SYMBOL(hdmi_ti_4xxx_transfer_enable);
