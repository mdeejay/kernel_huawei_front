...
audio_edid audio_fmt[HDMI_AUDIO_FORMAT_MAX_LENGTH];
int audio_channel_max = 2;
...
static int hdmi_set_timings(struct fb_videomode *vm, bool check_only)
{
	...
	for (i = 0; i < CEA_MODEDB_SIZE; i++) {
		if (relaxed_fb_mode_is_equal(cea_modes + i, vm)) {
             if (vm->xres ==1920 && vm->yres ==1080 && vm->refresh > 30) {
                 goto fail;
             }
             else {
...
{
	int ret = 0, i;
    int disp,number,current_byte,j;
    int ind = 0;
    int offset = 0;

    memset(audio_fmt, 0x0, HDMI_AUDIO_FORMAT_MAX_LENGTH);
	if (hdmi.edid_set)
		return hdmi.edid;
	...
	if (memcmp(hdmi.edid, edid_header, sizeof(edid_header)))
		return NULL;
    if (0x00 == hdmi.edid[0x7e]) {
        pr_info( "has NO extension block! \n");

    }else {
        disp = hdmi.edid[(0x80) + 2];
        if (0x4 == disp) {
            pr_info( "NO data is provided in the reserved data block! \n");

        }else {
            i = 0x80 + 0x4;

            while (i < (0x80 + disp)) {
            current_byte = hdmi.edid[i];

            if ((current_byte >> 5) == DATABLOCK_AUDIO) {
                offset = i;
                pr_info( "offset = %d\n", offset);
        ...
        for(j = 1 ; j<number;j++) {
            if ( 1 == j%3){
                current_byte = hdmi.edid[offset+j];
                audio_fmt[ind].format = (current_byte & 0x78) >> 3;
                audio_fmt[ind].num_of_ch = (current_byte & 0x07) + 1;
                audio_channel_max = audio_fmt[ind].num_of_ch;
                if (audio_fmt[ind].num_of_ch >= audio_channel_max) {
                    audio_channel_max = audio_fmt[ind].num_of_ch;
                }
                pr_info("audio_fmt[%d].format = %d, .num_of_ch = %d, audio_channel_max = %d\n",ind,audio_fmt[ind].format,audio_fmt[ind].num_of_ch,audio_channel_max);
            }else if (2 == j%3) {
                 audio_fmt[ind].sampling_freq = hdmi.edid[offset + j];
                 ind++;
            }
        }
     }
