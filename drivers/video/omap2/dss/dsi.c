...
/* 0003-Added-a-separate-function-for-mcs-and-dcs-commands.patch */
#define DSI_DT_MCS_SHORT_WRITE_1	0x23
#define DSI_DT_MCS_LONG_WRITE		0x29
/*  Patch end */
...
static inline int dsi_if_enable(struct platform_device *dsidev, bool enable)
{
	...
	enable = enable ? 1 : 0;
        /*Reason: The bit 0 of register DSI_CTRL is  written '0' unsuccessfully in probability, so
                            some command cannot send or receive in command mode
        Principle: When the bit 0 of register DSI_CTRL is written unsuccessfully, we will write again
                         and again until it failed 5 times,then we give up and return ERR*/
#if 0
...
again:
       ...
       if((wait_for_bit_change(dsidev, DSI_CTRL, 0, enable) != enable) && (count < maxtimes))
       {
            ...
            if(count == maxtimes)
            {
               DSSERR("Failed to set dsi_if_enable to %d %d times\n", enable,count);
               return -EIO;
            }
            goto again;
       }
...
#if 0
void omapdss_dsi_vc_enable_lp_cmd_mode(struct omap_dss_device *dssdev, int channel)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
	...
	REG_FLD_MOD(dsidev, DSI_VC_CTRL(channel), 0, 9, 9); // mode speed - LP mode;
...
/* 0003-Added-a-separate-function-for-mcs-and-dcs-commands.patch */
int dsi_vc_mcs_write_nosync(struct omap_dss_device *dssdev, int channel,
	u8 *data, int len)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
	int r;
...
EXPORT_SYMBOL(dsi_vc_mcs_write_nosync);
/* patch end */
...
/* 0003-Added-a-separate-function-for-mcs-and-dcs-commands.patch */
int dsi_vc_mcs_write(struct omap_dss_device *dssdev, int channel,
	u8 *data, int len)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
	int r;

	r = dsi_vc_mcs_write_nosync(dssdev, channel, data, len);
	if (r)
		goto err;
...
EXPORT_SYMBOL(dsi_vc_mcs_write);
/* patch end */
...
int dsi_vc_gen_read(struct omap_dss_device *dssdev, int channel, u16 cmd,
        u8 *buf, int buflen)
{
    struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
    struct dsi_data *dsi = dsi_get_dsidrv_data(dsidev);
    ...
    if (dsi->debug_read)
        DSSDBG("dsi_vc_gen_read(ch%d, mcs_cmd %x)\n", channel, cmd);

    r = dsi_vc_send_short(dsidev, channel, DSI_DT_GENERIC_READ_1, cmd, 0);
    if (r)
        goto err;
    ...
    if (dt == DSI_DT_RX_ACK_WITH_ERR) {
        ...
        int len = FLD_GET(val, 23, 8);
        if (dsi->debug_read)
            DSSDBG("\tMCS long response, len %d\n", len);

        if (len > buflen) {
            DSSDBG("\tMCS long response: returned len %d, not enough buf size\n", len);
            r = -EIO;
            goto err;
        }
    ...
    BUG();
err:
    DSSERR("dsi_vc_gen_read(ch %d, cmd 0x%02x) failed\n",
            channel, cmd);
    return r;

}
EXPORT_SYMBOL(dsi_vc_gen_read);
...
/*  Reason: Remove the hard coding for LCD display error on top line  */
	r = dsi_read_reg(dsidev, DSI_CTRL);
	r = FLD_MOD(r, 1, 0, 0);	/* TX_FIFO_ARBITRATION */
	r = FLD_MOD(r, 1, 3, 3);	/* TX_FIFO_ARBITRATION */
	r = FLD_MOD(r, 1, 4, 4);	/* VP_CLK_RATIO, always 1, see errata*/
	r = FLD_MOD(r, buswidth, 7, 6);	/* VP_DATA_BUS_WIDTH */
	r = FLD_MOD(r, 0, 8, 8);	/* VP_CLK_POL */
...
/*  Reason: Remove the hard coding for LCD display error on top line  */
	...
	r = FLD_VAL(0x7a67, 31, 16) |   /* BL_HS_INTERLEAVING */
		FLD_VAL(0x31d7, 15, 0); /* BL_LP_INTERLEAVING */
...
#if 0
	//printk("HARII: Setting ddr_prei=%d , ddr_post=%d, clocks to DSI tool config values",ddr_clk_pre,ddr_clk_post);
	ddr_clk_pre = 30;
	ddr_clk_post = 15;
#endif
...
/* Patch : 0001-Fix-for-supporting-interleaving-sending-the-non-vide.patch */
void omapdss_dsi_vc_enable_lp_cmd_mode(struct omap_dss_device *dssdev,
	int channel, int cmd)
{
	struct platform_device *dsidev = dsi_get_dsidev_from_dssdev(dssdev);
