...
static    bool_tt    deglitchingRsenNow = false;
#endif
//
...
//static bool    timer_SWWA_WRITE_STAT_Expired= false;
...
enum hrtimer_restart timer_SWWA_WRITE_STAT_callback( struct hrtimer *timer )
{
  timer_SWWA_WRITE_STAT_Expired = true;
   TX_DEBUG_PRINT(( "Drv: timer_SWWA_WRITE_STAT_Expired now!!!!!!!\n"));
  return HRTIMER_NORESTART;
}
#endif
...
//    I2C_WriteByte(PAGE_0_0X72, 0x96, 0x22);
    ...
    TX_DEBUG_PRINT(("Drv: D0:0x93:0x%02x\n",(int)ReadBytePage0(0x93)));
...
static void    ProcessRgnd( void )
{
    ...
    else
    {
        ...
        {
            // Release USB ID switch   changed by Garyyuan 20120120
			ReadModifyWritePage0(0x95, BIT6, 0x00);
        }
    }
}
...
void	USB_Plugout_SetToD3( void )
{
	if(fwPowerState != POWER_STATE_FIRST_INIT){
		TX_DEBUG_PRINT(("Drv: USB_ID should disconnected with CBUS_ID after USB pluged out.\n"));
		ForceUsbIdSwitchOpen();
...
static void	DetectRgndtoSetUSBID( void )
{
	...
	// 00, 01 or 11 means USB.
	// 10 means 1K impedance (MHL)
	if(0x03 == reg99RGNDRange){

		TX_DEBUG_PRINT(("Drv: USB_ID should connected with CBUS_ID after USB pluged in.\n"));
		...
		{
		TX_DEBUG_PRINT(("Drv: USB_ID should disconnected with CBUS_ID after MHL pluged out.\n"));
		ForceUsbIdSwitchOpen();
...
static void MhlTxDrvProcessConnection ( void )
{
    ...
    if(TRUE == get_mhl_connect())
    {
        ...
        TX_DEBUG_PRINT(("Drv: D0:0x93:0x%02x    0x94:0x%02x\n",(int)ReadBytePage0(0x93),(int)ReadBytePage0(0x94)));
    }
    //
...
#define CBUS_LINK_STATUS_2 0x38
    ...
    uint8_t address;
        ...
        printk(KERN_INFO  "Starting timer to fire in WWA_WRITE_STAT: %ldms (%ld)\n", delay_in_ms, jiffies );
