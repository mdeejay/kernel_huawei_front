...
static int omap_temp_sensor_pm_notifier_cb(struct notifier_block *notifier,
				unsigned long pm_event,  void *unused)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		if (cancel_delayed_work_sync
				(&temp_sensor_pm->omap_sensor_work)) {
	...
	case PM_POST_SUSPEND:
		suspend_state = 0;
		r = schedule_delayed_work(&temp_sensor_pm->omap_sensor_work,
				msecs_to_jiffies(0));
		if (r)
...
static struct notifier_block omap_die_pm_notifier = {
        .notifier_call = omap_temp_sensor_pm_notifier_cb,
};
...
static int __devinit omap_temp_sensor_probe(struct platform_device *pdev)
{
	...
	r = register_pm_notifier(&omap_die_pm_notifier);
	if (r)
		pr_err("%s: omap_temp sensor pm registration failed!\n", __func__);
