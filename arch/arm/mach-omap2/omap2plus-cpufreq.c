...
#include "dvfs.h"
#define CASE_INIT_COOLING_LEVEL 100
#ifdef CONFIG_SMP
struct lpj_info {
	unsigned long	ref;
	unsigned int	freq;
...
static int cpufreq_apply_cooling(struct thermal_dev *dev,
				int cooling_level)
{
	if (cooling_level >= CASE_INIT_COOLING_LEVEL)
		case_cooling_level = (cooling_level - CASE_INIT_COOLING_LEVEL);
	else
...
static int __init omap_cpufreq_cooling_init(void)
{
	...
	ret += thermal_cooling_dev_register(&case_thermal_dev);
	return ret;
}
