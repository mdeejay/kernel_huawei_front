...
static struct dentry *lpddr2_debugfs_dir;
extern unsigned int get_lpddr2_info(void);
#endif
...
#if defined(CONFIG_GET_LPDDR2_INFO)
static ssize_t lpddr2_info_read(struct file *filp,  char  __user *buffer,size_t count, loff_t *ppos)
{

    int len = 0;
    char info_buff[128],manufacturer[10];
    char lpddr2_type[4][10] = {"S4 SDRAM","S2 SDRAM","N NVM","Reserved"};
    char lpddr2_iowidth[4][10] = {"x32","x16","x8","not used"};
    char lpddr2_density[11][10] = {"64Mb","128Mb","256Mb","512Mb","1Gb","2Gb",\
                                   "4Gb","8Gb","16Gb","32Gb","reserved"};
    unsigned int lpddr2_hardware_info;

    memset(manufacturer,0,10);
    memset(info_buff, 0, 128);
    lpddr2_hardware_info = get_lpddr2_info();
    switch(lpddr2_hardware_info >> 8)
    {
        case 0x01:
            strcpy(manufacturer,"Samsung");
            break;
        case 0x02:
            strcpy(manufacturer,"Qimonda");
            break;
        case 0x03:
            strcpy(manufacturer,"Elipida");
            break;
        case 0x04:
            strcpy(manufacturer,"Etron");
            break;
        case 0x05:
            strcpy(manufacturer,"Nanya");
            break;
        case 0x06:
            strcpy(manufacturer,"Nynix");
            break;
        case 0x07:
            strcpy(manufacturer,"Mosel");
            break;
        case 0x08:
            strcpy(manufacturer,"Winbond");
            break;
        case 0x09:
            strcpy(manufacturer,"ESMT");
            break;
        case 0x0b:
            strcpy(manufacturer,"Spansion");
            break;
        case 0x0c:
            strcpy(manufacturer,"SST");
            break;
        case 0x0d:
            strcpy(manufacturer,"ZMOS");
            break;
        case 0x0e:
            strcpy(manufacturer,"Intel");
            break;
        case 0xfe:
            strcpy(manufacturer,"Numonyx");
            break;
        case 0xff:
            strcpy(manufacturer,"Micron");
            break;
        default:
            strcpy(manufacturer,"Reserved");
            break;
    }
    len = sprintf(info_buff, "LPDDR2 Information\nManufacturer:%s\nType:%s\nI/O width:%s\nDensity:%s\n", \
                  manufacturer,lpddr2_type[(lpddr2_hardware_info & 0xc0) >> 6],\
                  lpddr2_iowidth[lpddr2_hardware_info & 0x3],\
                  lpddr2_density[(lpddr2_hardware_info & 0x3c) >> 2]);

    return simple_read_from_buffer(buffer, count, ppos,(void *) info_buff, len);
}
static const struct file_operations lpddr2_info_fops = {
    .read = lpddr2_info_read,
};
int  setup_lpddr2_debugfs(void)
{
    struct dentry *lpddr2_file;

    lpddr2_debugfs_dir = debugfs_create_dir("lpddr2", NULL);
    if(!lpddr2_debugfs_dir)
        return -ENOENT;
    lpddr2_file = debugfs_create_file("lpddr2_info", 0444, lpddr2_debugfs_dir, NULL, &lpddr2_info_fops);
    return 0;
}
#endif
