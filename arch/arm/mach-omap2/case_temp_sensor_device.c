...
void case_temp_sensor_resume_idle(void)
{
}

void case_temp_sensor_prepare_idle(void)
{
}

static struct omap_device_pm_latency case_temp_sensor_latency[] = {
	{
...
static int case_temp_sensor_dev_init(struct omap_hwmod *oh, void *user)
{
	...
	od = omap_device_build(temp_sensor_pdata->name, i, oh,
			       ...
			       case_temp_sensor_latency,
			       ARRAY_SIZE(case_temp_sensor_latency), 0);
	if (IS_ERR(od)) {
